...
tabla_1_mod <- table(muertes_cr$grp_muerte, muertes_cr$regsalud)
kable(tabla_1_mod, caption = "")

tabla_2_mod <- table(muertes_cr$grp_muerte, muertes_cr$nacionalid)
kable(tabla_2_mod, caption = "")

tabla_3_mod <- table(muertes_cr$grp_muerte, muertes_cr$sexo)
kable(tabla_3_mod, caption = "")

tabla_4_mod <- table(muertes_cr$grp_muerte, muertes_cr$estcivil)
kable(tabla_4_mod, caption = "")

tabla_5_mod <- table(muertes_cr$grp_muerte, muertes_cr$ocuparec)
kable(tabla_5_mod, caption = "")
...
Con el conocimiento de cada uno de los parámetros, se procede a realizar las pruebas para las tablas de contingencias creadas anteriormente. Se inicia con la división de 17 grupos realizada por el INEC.

```{r Pruebas_independencia_INEC}

lista_stad <- list()

# Se inicia con las tablas con valores de grupo de muerte del INEC
...
Luego, se procede con la división de 5 grupos de enfermedades realizada por los integrantes del grupo.

```{r Pruebas_independencia_grp}

# Se continúa con los valores del grupo de muerte elaborado por el grupo de trabajo
lista_stad[[6]] <- chisq.test(x = tabla_1_mod, correct = TRUE)

lista_stad[[7]] <- chisq.test(x = tabla_2_mod, correct = TRUE)

lista_stad[[8]] <- chisq.test(x = tabla_3_mod, correct = TRUE)

lista_stad[[9]] <- chisq.test(x = tabla_4_mod, correct = TRUE)

lista_stad[[10]] <- chisq.test(x = tabla_5_mod, correct = TRUE)
